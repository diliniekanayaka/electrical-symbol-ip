...
def run(weights='yolov5s.pt',  # model.pt path(s)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    # print("Class predictions", names[int(c)] , f"{n}"  )
                    model_predictions[names[int(c)]] =  f"{n}"
    ...
    item_prices = {"Item":[], "Price": [], "Quantity": [], "Total": []}
    for idx in model_predictions:
      quantity =  model_predictions[idx]
      price =  price_sheet.loc[price_sheet["Item"] == idx]['Price'].values[0]
      total = float(quantity) * float(price)
      item_prices["Item"].append(idx)
      item_prices["Price"].append(price)
      item_prices["Quantity"].append(quantity)
      item_prices["Total"].append(total)
